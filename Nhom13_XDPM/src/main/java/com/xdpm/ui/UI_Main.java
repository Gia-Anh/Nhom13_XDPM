...
public class UI_Main extends JFrame{
	...
	public UI_Main() {
		...
		jmiThanhToan.addActionListener(e ->{
			tabbedPane.removeAll();
			tabbedPane.addTab("Thanh toán trễ hạn", new ImageIcon("image/money.png"), pnlTTTreHan, "Thanh toán trễ hạn");
		});
		
		pnlDatTruoc = new UI_DatTruoc();
		jmiThemDatTruoc.addActionListener(e ->{
			tabbedPane.removeAll();
			tabbedPane.addTab("Đặt trước tựa đề", new ImageIcon("image/ThemDatTruoc.png"), pnlDatTruoc, "Đặt trước tựa đề");
		});
	...
	public void changeUI(boolean isManager) {
		...
		if (isManager) {
			...
			jmiCapNhatNgayThue = new JMenuItem("Cập nhật số ngày thuê", new ImageIcon("image/update.png"));
